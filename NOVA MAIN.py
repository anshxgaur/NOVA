...
def plot_graph():
    speak("What should I plot? You can say 'sine wave' or 'manual coordinates'.")
    ...
    if "sine" in choice:
        ...
        plt.title("Sine Wave")
        plt.show()
        speak("Plotting the sine wave now.")
    elif "coordinates" in choice or "manual" in choice:
        speak("Please type the X coordinates separated by commas in the console.")
        x_input = input("Enter X values (e.g., 1,2,3): ")
        speak("Please type the Y coordinates separated by commas in the console.")
        y_input = input("Enter Y values (e.g., 4,5,6): ")
        try:
            x = [float(i) for i in x_input.split(',')]
            y = [float(i) for i in y_input.split(',')]
            plt.plot(x, y, marker='o')
            plt.show()
            speak("Graph plotted successfully.")
        except Exception:
            speak("Invalid coordinate format.")
...
# --- NEW: File & Folder Management ---
def open_folder(folder_path):
    # Expands user paths like 'Downloads'
    path = os.path.expanduser(folder_path)
    if os.path.exists(path):
        if system == "Windows":
            os.startfile(path)
        else:
            subprocess.show([ "open" if system == "Darwin" else "xdg-open", path])
        speak(f"Opening folder at {folder_path}")
    else:
...
def play_spotify():
    try:
        ...
        sp = spotipy.Spotify(auth_manager=SpotifyOAuth(client_id=SPOTIPY_CLIENT_ID,
                                               ...
                                               scope=scope))
        sp.start_playback()
        speak("Playing your Spotify music.")
    except Exception as e:
        speak("Spotify error. Make sure an instance of Spotify is open and your credentials are set.")
